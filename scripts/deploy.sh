...
RESPONSE_CODE=$(curl -s -o /dev/null -w "%{http_code}" http://3.37.106.173/api/port)
...
then
    ...
    CURRENT_PORT=$(curl -s http://3.37.106.173/api/port)
...
sudo docker pull juhwan0815/delivery-moa:latest

echo "> 도커 실행 포트:$IDLE_PORT"
sudo docker run -d --name $IDLE_PORT -p ${IDLE_PORT}:${IDLE_PORT} -e "server.port=${IDLE_PORT}" juhwan0815/delivery-moa:latest
